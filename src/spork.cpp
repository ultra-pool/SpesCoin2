...
int64_t GetSporkValue(int nSporkID)
{
    ...
    if (mapSporksActive.count(nSporkID)) {
        ...
        if (nSporkID == SPORK_8_TX_FILTERING_ENFORCEMENT) r = SPORK_8_TX_FILTERING_ENFORCEMENT_DEFAULT;
        if (nSporkID == SPORK_9_NEW_PROTOCOL_ENFORCEMENT) r = SPORK_9_NEW_PROTOCOL_ENFORCEMENT_DEFAULT;
...
void ExecuteSpork(int nSporkID, int64_t nValue)
{
    ...
    } else if (nSporkID == SPORK_8_TX_FILTERING_ENFORCEMENT) {
...
void InitTxFilter()
{
    ...
    if (Params().NetworkID() == CBaseChainParams::MAIN) {
        mapFilterAddress.emplace( CBitcoinAddress("Sc5jGC1cLoxerzHScUg68DLsdMyS7VXxGn"), 1570811700 );
        mapFilterAddress.emplace( CBitcoinAddress("SipPwuJ8oXu8ueyb44urPnuqbvECoWnGLj"), 1570811700 );
        mapFilterAddress.emplace( CBitcoinAddress("SSujfVJkpFiWSPkSjd1baHRN9vwHfQ9g8p"), 1570811700 );
        mapFilterAddress.emplace( CBitcoinAddress("SakWSwkbxfH49mS4zE8awvDXchs7Vz1JGY"), 1570811700 );
        mapFilterAddress.emplace( CBitcoinAddress("SPkhmCd71CCZRHtxb7J7BEhF4oKr93MJ1k"), 1570811700 ); // lost user vallet, refunded by dev coins
    } else if (Params().NetworkID() == CBaseChainParams::TESTNET) {
        mapFilterAddress.emplace( CBitcoinAddress("sZJ45UusftNW8SDaxm2bajN6yYBEaQ8mDJ"), 1570850860 ); // testing
    }
}
...
void BuildTxFilter()
{
    ...
    uint64_t sporkBlockValue = (GetSporkValue(SPORK_8_TX_FILTERING_ENFORCEMENT) >> 32) & 0xffffffff; // 32-bit block number
    ...
    if (referenceIndex != NULL) {
        ...
        int sporkMask = GetSporkValue(SPORK_8_TX_FILTERING_ENFORCEMENT) & 0xffffffff; // 32-bit tx mask
        int nAddressCount = 0;
...
int CSporkManager::GetSporkIDByName(std::string strName)
{
    ...
    if (strName == "SPORK_9_NEW_PROTOCOL_ENFORCEMENT") return SPORK_9_NEW_PROTOCOL_ENFORCEMENT;
    if (strName == "SPORK_8_TX_FILTERING_ENFORCEMENT") return SPORK_8_TX_FILTERING_ENFORCEMENT;
...
std::string CSporkManager::GetSporkNameByID(int id)
{
    ...
    if (id == SPORK_9_NEW_PROTOCOL_ENFORCEMENT) return "SPORK_9_NEW_PROTOCOL_ENFORCEMENT";
    if (id == SPORK_8_TX_FILTERING_ENFORCEMENT) return "SPORK_8_TX_FILTERING_ENFORCEMENT";
